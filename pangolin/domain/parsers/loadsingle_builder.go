...
func (app *loadSingleBuilder) Now() (LoadSingle, error) {
	if app.internal == "" {
		return nil, errors.New("the internal name is mandatory in order to build a LoadSingle instance")
	}

	if app.external == "" {
		return nil, errors.New("the external name is mandatory in order to build a LoadSingle instance")
	}
