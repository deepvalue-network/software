...
// Now builds a new Definition instance
func (app *builder) Now() (Definition, error) {
	if app.tokens == "" {
		return nil, errors.New("the tokens path is mandatory in order to build a Definition instance")
	}

	if app.root == "" {
		return nil, errors.New("the root pattern is mandatory in order to build a Definition instance")
	}

	if app.rules == "" {
		return nil, errors.New("the rules path is mandatory in order to build a Definition instance")
	}

	if app.logics == "" {
		return nil, errors.New("the logics path is mandatory in order to build a Definition instance")
	}
	...
	if app.patternMatches == nil {
		return nil, errors.New("the []PatternMatch are mandatory in order to build a Definition instance")
	}

	if app.input == "" {
		return nil, errors.New("the input variable is mandatory in order to build a Definition instance")
	}
