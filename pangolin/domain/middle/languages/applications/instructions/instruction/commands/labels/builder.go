...
func (app *builder) Now() (Label, error) {
	...
	if app.variable == "" {
		return nil, errors.New("the variable is mandatory in order to build a Label instance")
	}
	...
	if app.instructions == nil {
		return nil, errors.New("there must be at least 1 Instruction in order to build a Label instance")
	}

	return createLabel(app.name, app.variable, app.instructions), nil
}
